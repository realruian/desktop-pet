...
一只基于像素素材做的 Mac 桌面宠物：会**安静休息（带呼吸感）**、**眼睛跟随鼠标**、偶尔**随机走动**、**可以拖动**；还能**接入 Claude Code 显示任务状态**，以及**把文件拖给它一键在终端启动 Claude Code**。窗口透明、无边框、永远置顶，空白处点击穿透到下面的应用。
...
| 🚶 **随机走动** | 休息一阵后，做 1–2 个小动作：走到附近某处 / 挠头 / 叫 / 躺下打滚，然后继续休息 |
...
`renderer/pet.js` 顶部：`WIN`(窗口大小)、`WALK_SPEED`、`REST_MIN/MAX`(休息时长 → 控制休息占比)、
`BREATH_AMT/PERIOD`(呼吸)、`GAZE_*`(眼睛跟随)、`ACTIVITY_WEIGHTS`(动作权重)。

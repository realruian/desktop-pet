...
| Claude Code 状态 | 仓库内的 Claude 会话**开箱即用**（仓库根 `.claude/settings.json` 已内置 hooks）。想监听**所有项目**：把这份 hooks 同样地复制到全局 `~/.claude/settings.json` 即可 |

> 首次用「拖文件起终端」时，macOS 会请求**自动化**（控制 Terminal）与**辅助功能**（把 `@文件名` 预填进输入框）权限，各允许一次即可。Claude Code hooks 改完后，**已经开着的 Claude 会话需重启才会被监听**。
